         ...
         unsigned char *outputs)
{
	...
	if(sres<0)
		ast_log(LOG_ERROR,"hid_set_outputs() fail\n");
}
...
static int pmr_proc(struct chan_urd_pvt *o)
{
	...
	if(o->txkeyed||o->txtestkey)
	{
		...
		{
			o->pmrChan->txPttIn=1;
			if(o->debuglevel) ast_log(LOG_NOTICE,"[%s] txPttIn = %i\n",o->name,o->pmrChan->txPttIn);
		}
	}
...
static int pmr_proc_ctl(struct chan_urd_pvt *o)
{
	...
	if(o->owner==NULL){
		if(o->debuglevel>3) ast_log(LOG_NOTICE,"[%s] has no owner\n",o->name);
		return (-1);
	}
	...
	if(cd!=o->rxcarrierdetect)
	{
		...
		if(o->debuglevel) ast_log(LOG_NOTICE,"[%s] rxcarrierdetect = %i\n",o->name,cd);
	...
	{
		if(o->debuglevel)ast_log(LOG_NOTICE,"[%s] rxdcsdecode = %s\n",o->name,o->pmrChan->rxctcssfreq);
	...
	if ( cd && sd )
	{
		...
		if(!o->rxkeyed && o->debuglevel)ast_log(LOG_NOTICE,"[%s] o->rxkeyed = 1\n", o->name);
		o->rxkeyed = 1;
	}
	else
	{
		//if(o->rxkeyed)o->pmrChan->dd.b.doitnow=1;
		if(o->rxkeyed && o->debuglevel)ast_log(LOG_NOTICE,"[%s] o->rxkeyed = 0\n",o->name);
		o->rxkeyed = 0;
	}
...
static int urd_indicate(struct ast_channel *c, int cond, const void *data, size_t datalen)
{
	...
	switch (cond) {
		...
		case AST_CONTROL_RADIO_KEY:
			o->txkeyed = 1;			
			if(o->debuglevel)
				ast_log(LOG_NOTICE,"[%s] ACRK code=%s TX ON\n",o->name,(char *)data);
		...
		case AST_CONTROL_RADIO_UNKEY:
			o->txkeyed = 0;
			if(o->debuglevel)
				ast_log(LOG_NOTICE,"[%s] ACRUK TX OFF\n",o->name);
			if(o->b.forcetxcode)
			{
				o->b.forcetxcode=0;
